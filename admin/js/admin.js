...
// ==========================================
// ANALYTICS TAB — Google Analytics style overview:
// trend stat cards, a 14-day activity area chart, a read/unread donut,
// a content-breakdown bar list, and a recent-activity feed.
// ==========================================
...
// Renders a smooth-ish area+line chart from an array of numeric values.
...
// Renders a donut chart from [{label, value, color}] segments.
...
// Pulls one dimension + N metrics out of a GA4 runReport response row set.
function parseGA4Rows(report, metricIndexes = [0]) {
  return (report?.rows || []).map(r => ({
    dim: r.dimensionValues?.[0]?.value ?? "",
    metrics: metricIndexes.map(i => Number(r.metricValues?.[i]?.value || 0)),
  }));
}

function ga4DateToLabel(yyyymmdd) {
  const y = yyyymmdd.slice(0, 4), m = yyyymmdd.slice(4, 6), d = yyyymmdd.slice(6, 8);
  return new Date(`${y}-${m}-${d}T00:00:00`).toLocaleDateString("en-US", { day: "numeric", month: "short" });
}

const DEVICE_COLORS = { desktop: "#4f8dff", mobile: "#e6394f", tablet: "#35c98f" };
...
async function renderAnalyticsTab() {
  ...
  try {
    ...
    content.innerHTML = `
      <div class="analytics-setup-notice">
        <h3>Live analytics isn't connected yet</h3>
        <p>${esc(err.message || "Couldn't reach the analytics function.")}</p>
        <p>Make sure the <code>analytics</code> Netlify function is deployed and that
        <code>GA4_PROPERTY_ID</code> and <code>GA4_SERVICE_ACCOUNT_JSON</code> are set
        in Netlify → Site configuration → Environment variables.</p>
      </div>`;
    return;
  }

  const { dailyReport, summaryReport, countryReport, pageReport, deviceReport } = data;

  // --- 14-day trend chart ---
  const dailyRows = parseGA4Rows(dailyReport, [0, 1]);
  const dayLabels = dailyRows.map(r => ga4DateToLabel(r.dim));
  const usersSeries = dailyRows.map(r => r.metrics[0]);

  // --- current vs previous 28-day period ---
  const summaryRows = summaryReport?.rows || [];
  const current = summaryRows.find(r => r.dimensionValues?.[0]?.value === "date_range_0");
  const previous = summaryRows.find(r => r.dimensionValues?.[0]?.value === "date_range_1");
  const metricAt = (row, i) => (row ? Number(row.metricValues?.[i]?.value || 0) : 0);

  const curUsers = metricAt(current, 0), prevUsers = metricAt(previous, 0);
  const curSessions = metricAt(current, 2), prevSessions = metricAt(previous, 2);
  const curViews = metricAt(current, 3), prevViews = metricAt(previous, 3);
  const curAvgDuration = metricAt(current, 4);
  const curBounce = metricAt(current, 5);

  const usersTrend = computeTrend(curUsers, prevUsers);
  const sessionsTrend = computeTrend(curSessions, prevSessions);
  const viewsTrend = computeTrend(curViews, prevViews);

  // --- top countries / pages ---
  const countryRows = parseGA4Rows(countryReport, [0]);
  const maxCountry = Math.max(1, ...countryRows.map(r => r.metrics[0]), 0);
  const countryHtml = countryRows.length ? countryRows.map(r => `
    <div class="analytics-bar-row">
      <span class="analytics-bar-label">${esc(r.dim || "Unknown")}</span>
      <div class="analytics-bar-track"><div class="analytics-bar-fill" style="width:${(r.metrics[0] / maxCountry * 100).toFixed(1)}%"></div></div>
      <span class="analytics-bar-value">${r.metrics[0]}</span>
    </div>`).join("") : `<p style="color:var(--text-muted);text-align:center;padding:1rem;">No visitors yet.</p>`;

  const pageRows = parseGA4Rows(pageReport, [0]);
  const maxPage = Math.max(1, ...pageRows.map(r => r.metrics[0]), 0);
  const pageHtml = pageRows.length ? pageRows.map(r => `
    <div class="analytics-bar-row">
      <span class="analytics-bar-label" title="${esc(r.dim)}">${esc(r.dim || "/")}</span>
      <div class="analytics-bar-track"><div class="analytics-bar-fill" style="width:${(r.metrics[0] / maxPage * 100).toFixed(1)}%"></div></div>
      <span class="analytics-bar-value">${r.metrics[0]}</span>
    </div>`).join("") : `<p style="color:var(--text-muted);text-align:center;padding:1rem;">No pageviews yet.</p>`;

  // --- device breakdown donut ---
  const deviceRows = parseGA4Rows(deviceReport, [0]);
  const donutSegments = deviceRows.length
    ? deviceRows.map(r => ({ label: r.dim, value: r.metrics[0], color: DEVICE_COLORS[r.dim?.toLowerCase()] || "#8a8a94" }))
    : [{ label: "No data", value: 1, color: "#3a3a42" }];

  const lineChart = buildAreaChartSVG(usersSeries.length ? usersSeries : [0], dayLabels.length ? dayLabels : ["—"], "#4f8dff");
  const donutChart = buildDonutSVG(donutSegments);

  content.innerHTML = `
    <p class="dash-subtitle">Live visitor data from Google Analytics — last 28 days.</p>

    <div class="analytics-stats-grid">
      <div class="analytics-stat-card">
        <span class="analytics-stat-label">Visitors (28d)</span>
        <span class="analytics-stat-value">${curUsers}</span>
        <span class="analytics-trend ${usersTrend.up ? "trend-up" : "trend-down"}">${usersTrend.up ? "▲" : "▼"} ${usersTrend.pct}% vs prior 28d</span>
      </div>
      <div class="analytics-stat-card">
        <span class="analytics-stat-label">Page Views (28d)</span>
        <span class="analytics-stat-value">${curViews}</span>
        <span class="analytics-trend ${viewsTrend.up ? "trend-up" : "trend-down"}">${viewsTrend.up ? "▲" : "▼"} ${viewsTrend.pct}% vs prior 28d</span>
      </div>
      <div class="analytics-stat-card">
        <span class="analytics-stat-label">Sessions (28d)</span>
        <span class="analytics-stat-value">${curSessions}</span>
        <span class="analytics-trend ${sessionsTrend.up ? "trend-up" : "trend-down"}">${sessionsTrend.up ? "▲" : "▼"} ${sessionsTrend.pct}% vs prior 28d</span>
      </div>
      <div class="analytics-stat-card">
        <span class="analytics-stat-label">Avg. Session</span>
        <span class="analytics-stat-value">${Math.round(curAvgDuration)}s</span>
        <span class="analytics-trend">${curBounce.toFixed(1)}% bounce rate</span>
      </div>
    </div>

    <div class="analytics-grid">
      <div class="analytics-panel analytics-panel-wide">
        <h3 class="analytics-panel-title">Visitors — last 14 days</h3>
        <div class="chart-wrap">${lineChart}</div>
      </div>
      <div class="analytics-panel">
        <h3 class="analytics-panel-title">Visitors by device</h3>
        <div class="donut-wrap">
          ${donutChart}
          <div class="donut-legend">
            ${donutSegments.map(s => `<span><i style="background:${s.color}"></i> ${esc(s.label)} (${s.value})</span>`).join("")}
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="analytics-grid">
      <div class="analytics-panel">
        <h3 class="analytics-panel-title">Top countries</h3>
        <div class="analytics-bar-list">${countryHtml}</div>
      </div>
      <div class="analytics-panel">
        <h3 class="analytics-panel-title">Top pages</h3>
        <div class="analytics-bar-list">${pageHtml}</div>
      </div>
    </div>

    <p class="analytics-source-note">Source: Google Analytics 4 (real-time via secure server function)</p>
  `;
}
